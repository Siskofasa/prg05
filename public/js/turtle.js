...
(function () {
  ...
  function TurtleCanvas(options) {
    ...
    this.context = this.canvas.getContext('2d'); //this.canvas.style.display = 'block';
  ...
  TurtleCanvas.prototype.setworldcoordinates = function (llx, lly, urx, ury) {
    ...
    if (lly == 0) this.context.translate(-llx, lly - (ury - lly));else if (lly > 0) this.context.translate(-llx, -lly * 2);else this.context.translate(-llx, -ury);
  ...
  TurtleCanvas.prototype.bgcolor = function (c) {
    this.background_color = c; //this.canvas.style.setProperty("background-color", c.v);
  ...
  TurtleCanvas.prototype.tracer = function (t, d) {
    ...
    if (d !== undefined) this.setDelay(d);
  ...
  render = function (_render) {
    ...
    TurtleGraphics.canvasLib[TurtleGraphics.defaults.canvasID].clearRect(llx, lly, urx - llx, ury - lly); //canvas.style.setProperty("background-color",TurtleGraphics.turtleCanvas.bgcolor.v);
    ...
    for (var tix in TurtleGraphics.turtleList) {
      ...
      if (t.aCount >= t.drawingEvents.length) t.aCount = t.drawingEvents.length - 1;
      moveTo(0, 0);
      ...
      var filling = false;
      if (isNaN(t.turtleCanvas.delay)) t.turtleCanvas.delay = 0; //				console.log(tix + " : " + t.clearPoint + " to " + t.aCount)
      ...
      for (var i = t.clearPoint; (i <= t.aCount || t.turtleCanvas.delay == 0) && i < t.drawingEvents.length; i++) {
        ...
        var ts = oper[oper.length - 1]; //					console.log(i + "/" + ts + oper [0] + "{" + oper [1] + "}" + t.turtleCanvas.delay)
        ...
        if (ts <= TurtleGraphics.renderClock || t.turtleCanvas.delay == 0) {
          if (ts > TurtleGraphics.renderClock) //	If we go past the render clock, jump it ahead
            TurtleGraphics.renderClock = ts; //						console.log("<==")
          ...
          if (oper[0] == "LT") {
            ...
            // RNL delay
            var df = oper[1]; //                      	console.log("animated delay set " + df)
            ...
            t.turtleCanvas.delay = df; //	t.turtleCanvas.intervalId = setInterval(render, t.turtleCanvas.delay)
  ...
  Turtle.prototype.go_home = function () {
    ...
    context.moveTo(this.home[0], this.home[1]);
    this.heading = new Vector([1.0, 0.0, 0.0]); // to the right; in turtle space x+ direction

    this.normal = new Vector([0.0, 0.0, -1.0]); // in z- direction
  };
  ...
  Turtle.prototype.initialize = function () {
    ...
    if (TurtleGraphics.canvasInit == false) {
      ...
      this.context.save();
      this.context.translate(this.context.canvas.width / 2, this.context.canvas.height / 2); // move 0,0 to center.
      ...
      this.context.clear_canvas(this.canvasID);
    } //TODO: Does this need to be fixed?


    this.context.turtleCanvas = TurtleGraphics.canvasLib[this.canvasID];
    this.context.home = new Vector([0.0, 0.0, 0.0]);
    this.context.visible = true;
    this.context.shapeStore = {};
    this.context.shapeStore['turtle'] = turtleShapePoints();
    this.context.shapeStore['arrow'] = defaultShapePoints();
    this.context.shapeStore['circle'] = circleShapePoints();
    this.context.shapeStore['square'] = squareShapePoints();
    this.context.shapeStore['triangle'] = triangleShapePoints();
    this.context.shapeStore['blank'] = [new Vector(0, 0)];
    this.context.shapeStore['classic'] = classicShapePoints();
    this.context.currentShape = 'classic';
    this.context.drawingEvents = [];
    this.context.filling = false;
    this.context.pen = true;
    this.context.penStyle = 'black';
    this.context.penWidth = 2;
    this.context.fillStyle = 'black';
    this.context.position = [];
    this.context.heading = [];
    this.context.normal = [];
    this.context.go_home();
    this.context.aCount = 0;
    this.context.clearPoint = 0; // RNL for clear/clearScreen
  };

  function turtleShapePoints() {
    var pl = [[0, 16], [-2, 14], [-1, 10], [-4, 7], [-7, 9], [-9, 8], [-6, 5], [-7, 1], [-5, -3], [-8, -6], [-6, -8], [-4, -5], [0, -7], [4, -5], [6, -8], [8, -6], [5, -3], [7, 1], [6, 5], [9, 8], [7, 9], [4, 7], [1, 10], [2, 14]];
    var res = [];
  ...
  function circleShapePoints() {
    var pl = [[10, 0], [9.51, 3.09], [8.09, 5.88], [5.88, 8.09], [3.09, 9.51], [0, 10], [-3.09, 9.51], [-5.88, 8.09], [-8.09, 5.88], [-9.51, 3.09], [-10, 0], [-9.51, -3.09], [-8.09, -5.88], [-5.88, -8.09], [-3.09, -9.51], [-0.00, -10.00], [3.09, -9.51], [5.88, -8.09], [8.09, -5.88], [9.51, -3.09]];
    var res = [];
  ...
  function triangleShapePoints() {
    var pl = [[10, -5.77], [0, 11.55], [-10, -5.77]];
    var res = [];
  ...
  function squareShapePoints() {
    var pl = [[10, -10], [10, 10], [-10, 10], [-10, -10]];
    var res = [];
  ...
  Turtle.prototype.clean = function () {
    ...
    if (arguments.length >= 1) {
      this.clear_canvas(this.canvasID, arguments[0]);
    } else {
  ...
  segmentLine = function segmentLine(sp, ep, sL, pen) {
    ...
    if (!(oldp[0] == ep[0] && oldp[1] == ep[1])) res.push([op, oldp[0], oldp[1], ep[0], ep[1]]);
    return res;
  };
  ...
  Turtle.prototype.draw_line = function (newposition) {
    if (!animate) {
      ...
      this.context.lineWidth = this.context.get_pen_width();
      this.context.strokeStyle = this.context.penStyle;
      this.context.lineTo(newposition[0], newposition[1]);
      this.context.stroke();
      if (!filling) this.context.closePath();
    } else {
      var r = segmentLine(this.context.position, newposition, turtleCanvas.getSegmentLength(), this.context.pen);

      for (var s in r) {
        r[s].push(penStyle);
        this.context.addDrawingEvent(r[s]);
      }

      if (!turtleCanvas.isAnimating()) {
        turtleCanvas.startAnimating(this);
      } else {
        if (!turtleCanvas.onCanvas(this)) turtleCanvas.addToCanvas(this);
      }
    }
  };

  Turtle.prototype.forward = function (d) {
    var newposition = position.linear(1, d, this.heading);
  ...
  Turtle.prototype.teleport_to = function (nx, ny) {
    if (nx instanceof Vector) var newposition = nx;else var newposition = new Vector([nx, ny, 0]);
  ...
  Turtle.prototype["goto"] = function (nx, ny) {
    ...
    if (this.pen) {
      ...
      if (!animate) {
        ...
        var r = segmentLine(position, newposition, turtleCanvas.getSegmentLength(), this.pen);
  ...
  Turtle.prototype.turn = function (phi) {
    ...
    var left = this.normal.cross(this.heading);
    var newheading = heading.rotateNormal(left, this.normal, alpha);
    this.heading = newheading;

    if (this.animate) {
      this.addDrawingEvent(["TT", heading]);
    }
  };
  ...
  Turtle.prototype.set_heading = function (newhead) {
    if (_typeof(newhead).toLowerCase() === 'number') {
  ...
  Turtle.prototype.towards = function (to, y) {
    ...
    if (_typeof(to).toLowerCase() === 'number') {
  ...
  Turtle.prototype.distance = function (to, y) {
    if (_typeof(to).toLowerCase() === 'number') to = new Vector(to, y, 0);
  ...
  Turtle.prototype.dot = function () {
    ...
    var color = this.context.penStyle;
    var nc = arguments[1] || color;

    if (!this.context.animate) {
      this.context.fillStyle = nc;
      this.context.fillRect(this.context.position[0] - size / 2, this.context.position[1] - size / 2, size, size);
      this.context.fillStyle = color;
    } else {
      this.addDrawingEvent(["DT", size, nc, this.context.position[0], this.context.position[1]]);
    }
  };
  ...
  Turtle.prototype.circle = function (radius, extent) {
    ...
    if (this.animate) {
      ...
      if (arcLen <= segLen) {
        ...
        if (Math.abs(extentLeft) > 0.01) this.arc(radius, extentLeft);
      }
    } else {
      this.arc(radius, extent);
    }
  ...
  Turtle.prototype.arc = function (radius, extent) {
    ...
    var ty = this.position[1]; //	Figure out the circle center

    var cx = tx + radius * Math.cos((turtleHeading + 90) * Degree2Rad);
    var cy = ty + radius * Math.sin((turtleHeading + 90) * Degree2Rad); //	Canvas arc angles go CLOCKWISE, not COUNTERCLOCKWISE like Turtle
    ...
    endAngleDeg = 360 - endAngleDeg; //	Becuase the y axis has been flipped in HTML5 Canvas with a tanslation, we need to adjust the angles

    startAngleDeg = -startAngleDeg;
    endAngleDeg = -endAngleDeg; //	Convert to radians

    var startAngle = startAngleDeg * Degree2Rad;
    var endAngle = endAngleDeg * Degree2Rad; //	Do the drawing
    ...
    if (extent && extent % 360 != 0) {
      ...
      this.teleport_to(nx, ny); //	If we're animating the turtle, move it on the screen
  ...
  Turtle.prototype.set_pen_color = function (c, g, b) {
    if (typeof c == "string") {
      ...
      if (_typeof(c) == "object" && c.length == 3) {
    ...
    if (this.animate) this.addDrawingEvent(["TC", c]);
  };
  ...
  Turtle.prototype.shape = function (s) {
    if (this.shapeStore[s]) this.currentShape = s;else {}
  };
  ...
  function clear_canvas(canId) {
    var documentContext = document.getElementById(canId).getContext('2d');

    if (arguments.length >= 2) {//		fillStyle = arguments[1];
      //		fillRect(0, 0, canvas.width, canvas.height);
    }

    documentContext.clearRect(-documentContext.canvas.width / 2, -documentContext.canvas.height / 2, documentContext.canvas.width, documentContext.canvas.height);
  } // Create a 3d Vector class for manipulating turtle heading, and position.


  function Vector(x, y, z) {
    if (_typeof(x).toLowerCase() === 'number') {
...
var $builtinmodule = function $builtinmodule(name) {
  var mod = {}; // First we create an object, this will end up being the class
  // class

  Sk.tg = TurtleGraphics;

  var checkArgs = function checkArgs(expected, actual, func) {
    if (actual != expected) {
      throw new Sk.builtin.TypeError(func + " takes exactly " + expected + " positional argument (" + actual + " given)");
    }
  };

  var turtle = function turtle($gbl, $loc) {
    ...
    $loc.update = new Sk.builtin.func(function (self) {//	Dummy function to emulate update... when not animating, we don't save the drawing operations, so this is pointless for us
    ...
    $loc.position = new Sk.builtin.func(function (self) {
      ...
      var x = new Sk.builtin.tuple([Sk.builtin.assk$(res[0], Sk.builtin.nmber.float$), Sk.builtin.assk$(res[1], Sk.builtin.nmber.float$)]);
      return x;
    });
    ...
    $loc.towards = new Sk.builtin.func(function (self, tx, ty) {
      ...
      if (_typeof(tx).toLowerCase() === 'number') {
        tx = [tx, ty, 0];
      } else {
    ...
    $loc.fillcolor = new Sk.builtin.func(function (self, color, green, blue) {
      ...
      } else return self.theTurtle.fillStyle;
    });
    $loc.pencolor = new Sk.builtin.func(function (self, color, green, blue) {
      ...
      } else return self.theTurtle.penStyle;
    });
    $loc.color = new Sk.builtin.func(function (self, color, green, blue) {
      ...
      } else return [self.theTurtle.penStyle, self.theTurtle.fillStyle];
    }); //
    //  Filling
    //
    ...
    $loc.fill = new Sk.builtin.func(function (self, fillt) {
      if (fillt === undefined) return self.theTurtle.filling;
      if (fillt) self.theTurtle.begin_fill();else self.theTurtle.end_fill();
    }); //
  ...
  var screen = function screen($gbl, $loc) {
    $loc.__init__ = new Sk.builtin.func(function (self) {
      TurtleGraphics.defaults = {
        canvasID: Sk.canvas,
        animate: true,
    ...
    $loc.title = new Sk.builtin.func(function (self, titlestring) {// no op....
    });
    ...
    $loc.colormode = new Sk.builtin.func(function (self) {//	Empty function to emulate compatibility
    }); //        $loc.clear = new Sk.builtin.func(function(self) {
    //
    //        });

    var myfunc = function myfunc(self, width, height, startx, starty) {
  ...
  mod.update = new Sk.builtin.func(function (self) {//	Dummy function to emulate update... when not animating, we don't save the drawing operations, so this is pointless for us
  });
  return mod;
};
...
/***/ 3:
/*!******************************************!*\
  !*** multi ./resources/js/lib/turtle.js ***!
  \******************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\Users\aurel\Documents\GitHub\ParsonProject\resources\js\lib\turtle.js */"./resources/js/lib/turtle.js");
