...
(function ($, _) {
  ...
  var graders = {}; // Grader that will execute the code and check variable values after that
  ...
  var VariableCheckGrader = function VariableCheckGrader(parson) {
    this.parson = parson;
  };

  graders.VariableCheckGrader = VariableCheckGrader; // Executes the given Python code and returns an object with two properties:
  ...
  VariableCheckGrader.prototype.grade = function (studentcode) {
    ...
    $.each(parson.options.vartests, function (index, testdata) {
      ...
      if ("_error" in res) {
        testcaseFeedback += parson.translations.unittest_error(that.stripLinenumberIfNeeded(res._error, testdata.initcode, student_code));
        success = false;
        ...
        for (var j = 0; j < variables.length; j++) {
          ...
          if (variable === "__output") {
            ...
            testcaseFeedback += "<div class='" + (variableSuccess ? "pass" : "fail") + "'>";
            testcaseFeedback += parson.translations.variabletest_assertion(variable, expected_value, actual_value) + "</div>";
          }

          log_entry.variables[variable] = {
      ...
      feedback += "<div class='testcase " + (success ? "pass" : "fail") + "'><span class='msg'>" + testdata.message + "</span><br>" + testcaseFeedback + "</div>";
    });
    return {
      html: feedback,
      tests: log_errors,
  ...
  var TurtleGrader = function TurtleGrader(p) {
    this.parson = p; // execute the model solution turtlet path to have the target "picture" visible in the
    // beginning

    var modelCommands = this._executeTurtleModel(); // specify variable tests for the commands executed by the student turtlet and the model
  ...
  graders.TurtleGrader = TurtleGrader; // copy the python execution functions from VariableCheckGrader

  TurtleGrader.prototype._python_exec = VariableCheckGrader.prototype._python_exec;
  TurtleGrader.prototype._variablesAfterExecution = VariableCheckGrader.prototype._variablesAfterExecution; // Execute the model turtlet code
  ...
  var UnitTestGrader = function UnitTestGrader(parson) {
    this.parson = parson;
  };

  graders.UnitTestGrader = UnitTestGrader; // copy the line number fixer and code-construction from VariableCheckGrader

  UnitTestGrader.prototype.stripLinenumberIfNeeded = VariableCheckGrader.prototype.stripLinenumberIfNeeded;
  UnitTestGrader.prototype._codelinesAsString = VariableCheckGrader.prototype._codelinesAsString; // copy the python executor from VariableCheckGrager

  UnitTestGrader.prototype._python_exec = VariableCheckGrader.prototype._python_exec; // do the grading
  ...
  UnitTestGrader.prototype.grade = function (studentcode) {
    var success = true,
        ...
        feedbackHtml = "",
        // HTML to be returned as feedback
    result,
        mainmod;

    var executableCode = studentCode + "\n" + unittests; // if there is code to add before student code, add it
    ...
    for (var i = 0, l = result.length; i < l; i++) {
      ...
      if (res.status === "error") {
        // errors in execution
        feedbackHtml += parson.translations.unittest_error(this.stripLinenumberIfNeeded(res._error, parson.options.unittest_code_prepend, studentCode));
        success = false;
      } else {
        // passed or failed tests
        feedbackHtml += '<span class="msg">' + this.stripLinenumberIfNeeded(res.feedback) + '</span><br />';
        feedbackHtml += 'Expected <span class="expected">' + res.expected + '</span>' + res.test + '<span class="actual">' + res.actual + '</span>';
  ...
  var LanguageTranslationGrader = function LanguageTranslationGrader(parson) {
    this.parson = parson;
  }; // Add the grader to the list of graders


  graders.LanguageTranslationGrader = LanguageTranslationGrader; // add open/close block definitions for pseudocode

  var langBlocks = {};
  LanguageTranslationGrader._languageBlocks = langBlocks; // specify the blocks for the pseudo language as a simple example case
  ...
  LanguageTranslationGrader.prototype.grade = function () {
    var student_code = this.parson.normalizeIndents(this.parson.getModifiedCode("#ul-" + this.parson.options.sortableId)); // Check opening and closing blocks.
    ...
    if (open && close) {
      ...
      var blocks = [],
          prevIndent = 0,
          // keep track of previous indent inside blocks
      ...
      for (i = 0; i < student_code.length; i++) {
        var isClose = false,
            // was a new blocks opened on this line
        isOpen = false,
            // was a block closed on this line
        item = student_code[i],
            line = $("#" + item.id).text(),
            // code of the line
        topBlock,
            bO; // Check if a proper indentation or the line was found in normalizeIndents
  ...
  LanguageTranslationGrader.prototype._replaceCodelines = function () {
    ...
    $.each(student_code, function (index, item) {
      var ind = parseInt(item.id.replace(parson.id_prefix, ''), 10); // Handle toggle elements. Expects the toggle areas in executable code to be marked
  ...
  var LineBasedGrader = function LineBasedGrader(parson) {
    this.parson = parson;
  };
  ...
  LineBasedGrader.prototype.grade = function (elementId) {
    ...
    var i;
    var wrong_order = false; // Find the line objects for the student's code
  ...
  var addToggleableElements = function addToggleableElements(widget) {
    ...
    context.off("click", ".jsparson-toggle").on("click", ".jsparson-toggle", function () {
      var $this = $(this),
          ...
          $parent = $this.parent("li"); // clear existing feedback

      widget.clearFeedback(); // change the shown toggle element
  ...
  var ParsonsCodeline = function ParsonsCodeline(codestring, widget) {
    this.widget = widget;
    this.code = "";
    this.indent = 0;
    this._toggles = [];
  ...
  window.ParsonsCodeline = ParsonsCodeline; // Creates a parsons widget. Init must be called after creating an object.

  var ParsonsWidget = function ParsonsWidget(options) {
    ...
    this.modified_lines = []; // contains line objects of distractors (see parseCode for line object description)

    this.extra_lines = []; // contains line objects (see parseCode for line object description)

    this.model_solution = []; //To collect statistics, feedback should not be based on this

    this.user_actions = []; //State history for feedback purposes
  ...
  ParsonsWidget._graders = graders; ////Public methods
  ...
  ParsonsWidget.prototype.parseCode = function (lines, max_distractors) {
    var distractors = [],
        ...
        lineObject,
        errors = [],
        that = this; // Create line objects out of each codeline and separate
  ...
  ParsonsWidget.prototype.init = function (text) {
    ...
    var id_prefix = this.id_prefix; // Add ids to the line objects in the user-draggable lines
  ...
  ParsonsWidget.prototype.addLogEntry = function (entry) {
    ...
    this.user_actions.push(logData); //Updating the state history
  ...
  ParsonsWidget.prototype.normalizeIndents = function (lines) {
    ...
    var match_indent = function match_indent(index) {
...
/***/ 2:
/*!*******************************************!*\
  !*** multi ./resources/js/lib/parsons.js ***!
  \*******************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\Users\aurel\Documents\GitHub\ParsonProject\resources\js\lib\parsons.js */"./resources/js/lib/parsons.js");
